...
from constants.llm import DEFAULT_ANTHROPIC_MODEL
from .prompts import (
...
async def generate_html_from_slide(
    ...
    """
    print(f"Generating HTML from slide image and XML using Anthropic...")
    try:
        ...
        try:
            from utils.oxml_parser import parse_oxml_to_structured_json
            structured_oxml = parse_oxml_to_structured_json(xml_content)
            user_text = f"STRUCTURED_OXML_JSON: \n{structured_oxml}\n{fonts_text}"
            print("Successfully parsed OXML into structured JSON for LLM.")
        except Exception as e:
            print(f"Failed to parse structured JSON: {e}")
            user_text = f"OXML: \n{xml_content}\n{fonts_text}"
        
        print("Making Anthropic API request for HTML generation...")
        ...
        if not html_content:
            raise HTTPException(status_code=500, detail="No HTML content generated by Anthropic")

        return html_content

    except APIError as e:
        print(f"Anthropic API Error: {e}")
        raise HTTPException(status_code=500, detail=f"Anthropic API error during HTML generation: {str(e)}")
    except Exception as e:
        error_msg = str(e)
        print(f"Exception occurred: {error_msg}")
        raise HTTPException(status_code=500, detail=f"Error during HTML generation: {error_msg}")
...
async def generate_react_component_from_html(
    ...
    try:
        ...
        print("Making Anthropic API request for React component generation...")
        ...
        react_content = response.content[0].text
        print(f"Received React content length: {len(react_content)}")

        if not react_content:
            raise HTTPException(status_code=500, detail="No React component generated by Anthropic")
        ...
        raise HTTPException(status_code=500, detail=f"Anthropic API error during React generation: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error during React generation: {str(e)}")
...
async def edit_html_with_images(
    ...
    try:
        ...
        print("Making Anthropic API request for HTML editing...")
        ...
        edited_html = response.content[0].text
        print(f"Received edited HTML content length: {len(edited_html)}")

        if not edited_html:
            raise HTTPException(status_code=500, detail="No edited HTML content generated by Anthropic")

        return edited_html

    except APIError as e:
        raise HTTPException(status_code=500, detail=f"Anthropic API error during HTML editing: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error processing HTML editing: {str(e)}")
