...
def _extract_slide_xmls(pptx_path: str, temp_dir: str) -> List[str]:
    ...
    try:
        ...
        # Extract slide size if available
        slide_size_comment = ""
        if os.path.exists(presentation_xml_path):
            try:
                ...
                if sld_sz is not None:
                    ...
                    cy = sld_sz.get("cy")
                    if cx and cy:
                        slide_size_comment = f'<!-- SLIDESIZE: cx="{cx}" cy="{cy}" -->\n'
            except Exception as e:
                print(f"Warning: Failed to extract slide size: {e}")
        ...
        for slide_file in slide_files:
            ...
            with open(slide_path, "r", encoding="utf-8") as f:
                slide_xmls.append(slide_size_comment + f.read())
...
async def _convert_pptx_to_pdf(pptx_path: str, temp_dir: str) -> str:
    ...
    try:
        ...
        if not libreoffice_exec and os.path.exists("/Applications/LibreOffice.app/Contents/MacOS/soffice"):
