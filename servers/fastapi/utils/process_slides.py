...
def _asset_path_to_url(path: str) -> str:
    """Convert an absolute filesystem image path to a browser-accessible URL."""
    return "/app_data/images/" + os.path.basename(path)
...
async def process_slide_and_fetch_assets(
    ...
    for image_path in image_paths:
        ...
        if isinstance(result, ImageAsset):
            ...
            image_dict["__image_url__"] = _asset_path_to_url(result.path)
        else:
    ...
    for i, new_image in enumerate(new_images):
        if new_images_fetch_status[i]:
            ...
            if isinstance(fetched_image, ImageAsset):
                ...
                image_url = _asset_path_to_url(fetched_image.path)
            else:
