...
async def get_slide_content_from_type_and_outline(
    ...
    try:
        ...
        response = await client.generate_structured(
            ...
            # Force Anthropic to always invoke ResponseSchema so that the full
            # slide content is returned, not just the speaker note.
            tool_choice={"type": "tool", "name": "ResponseSchema"} if client.llm_provider.value == "anthropic" else None
        )
        return response
    except Exception as e:
        print(f"DEBUG: LLM Generation Error: {str(e)}")
