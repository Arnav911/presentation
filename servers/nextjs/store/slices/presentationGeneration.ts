...
const presentationGenerationSlice = createSlice({
  ...
  reducers: {
    ...
    clearPresentationData: (state) => {
      if (state.presentationData) {
        state.presentationData = {
          id: state.presentationData.id || state.presentation_id || "",
          language: state.presentationData.language || "English",
          layout: state.presentationData.layout || { name: "", ordered: false, slides: [] },
          n_slides: 0,
          title: "",
          slides: [],
          theme: state.presentationData.theme
        };
      } else {
    ...
    setPresentationData: (state, action: PayloadAction<PresentationData>) => {
      if (state.presentationData) {
        // Merge the new data with existing data to preserve theme
        state.presentationData = {
          ...state.presentationData,
          ...action.payload,
          // Explicitly preserve theme if it exists in the current state but not in the payload
          theme: action.payload.theme || state.presentationData.theme
        };
      } else {
    ...
    setTheme: (state, action: PayloadAction<any>) => {
      if (state.presentationData) {
        ...
        // Initialize basic presentation data if it doesn't exist yet
        state.presentationData = {
    ...
    updateOutlineAtIndex: (state, action: PayloadAction<{ idx: number; content: string }>) => {
      if (state.outlines && state.outlines[action.payload.idx] !== undefined) {
        state.outlines[action.payload.idx] = { ...state.outlines[action.payload.idx], content: action.payload.content };
      }
    },
    // SLIDE OPERATIONS
    addSlide: (
