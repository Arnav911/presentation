...
export function SortableListItem({ slide, index, selectedSlide, onSlideClick }: SortableListItemProps) {
    ...
    return (
        <div
            ...
            {...listeners}
            onClick={handleClick}
            className={`p-3 cursor-pointer border-2 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md
                ${selectedSlide === index
                    ? ' border-white shadow-zinc-900 ring-4 ring-white/10'
                    : 'border-transparent hover:border-zinc-700'
                }`}
        >
            <span className={`font-medium slide-title ${selectedSlide === index ? 'text-white' : 'text-muted-foreground'}`}>Slide {index + 1}</span>
