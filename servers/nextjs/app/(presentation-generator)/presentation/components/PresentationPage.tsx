  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-background">
        <div
          className="bg-card border border-zinc-800 text-foreground px-6 py-8 rounded-lg shadow-lg flex flex-col items-center"
          role="alert"
        >
          ...
          <Button onClick={() => { trackEvent(MixpanelEvent.PresentationPage_Refresh_Page_Button_Clicked, { pathname }); window.location.reload(); }} className="bg-white text-black hover:bg-zinc-200 border-none">Refresh Page</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen flex overflow-hidden flex-col">
      <div className="fixed right-6 top-[5.2rem] z-50">
        {isSaving && <Loader2 className="w-6 h-6 animate-spin text-white" />}
      </div>
      ...
      <div
        className="flex flex-1 relative bg-background"
      >
        <SidePanel
        ...
        <div 
          className="flex-1 h-[calc(100vh-100px)] overflow-y-auto transition-colors duration-500 custom_scrollbar bg-background"
        >
          <div
            id="presentation-slides-wrapper"
            className="w-full mx-auto flex flex-col py-8 px-6 gap-8"
          >
            {!presentationData ||
            loading ||
