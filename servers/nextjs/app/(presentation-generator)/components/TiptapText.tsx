...
const TiptapText: React.FC<TiptapTextProps> = ({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: `outline-none focus:outline-none transition-all duration-200 ${className} ${!editable ? 'cursor-default' : ''}`,
  ...
  // Make editable property reactive
  useEffect(() => {
    if (!editor) return;
  ...
  return (
    <>
      ...
      <EditorContent
        editor={editor}
        className={`tiptap-text-editor w-full h-full min-h-[1em] ${editable ? 'cursor-text' : 'cursor-default'}`}
        style={{
