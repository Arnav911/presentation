...
const compileCustomLayout = (layoutCode: string, React: any, z: any) => {
  ...
  const codeWithoutExports = cleanCode
    ...
    .replace(/export\s+default\s+([a-zA-Z0-9$_]+);?/g, "")
    // Remove "export { Component }"
    .replace(/export\s*{[^}]+};?/g, "")
    // Remove "export const X" => "const X"
    .replace(/export\s+(const|let|var|function|class)/g, "$1");

  // Fix AI-generated out-of-order Zod chains (e.g., .default("x").min(5) -> .min(5).default("x"))
  let normalizedCode = codeWithoutExports;

  // Reorder .min() and .max() if they appear after .default() or .meta()
  // This helps when the LLM outputs z.string().default("").meta({...}).min(5)
  for (let i = 0; i < 3; i++) {
    normalizedCode = normalizedCode
      .replace(/\.(default|meta)\(([^)]*(?:\([^)]*\)[^)]*)*)\)\.(min|max)\(([^)]+)\)/g, ".$3($4).$1($2)")
      .replace(/\.(default|meta)\(\{([^}]*)\}\)\.(min|max)\(([^)]+)\)/g, ".$3($4).$1({$2})");
  }

  const compiled = Babel.transform(normalizedCode, {
    presets: [
...
export const LayoutProvider: React.FC<{
  ...
  const LoadCustomLayouts = async () => {
    ...
    try {
      ...
      if (!customTemplateResponse.ok) {
        console.warn("Failed to fetch custom templates summary");
        return {
          layoutsById,
          layoutsByTemplateID,
          templateSettings: templateSettingsMap,
      ...
      console.log("Custom Templates Data:", customTemplateData);
      ...
      for (const templateInfo of customTemplates) {
        ...
        if (!customLayoutResponse.ok) {
          console.warn(`Failed to fetch custom layouts for ${presentationId}`);
          continue;
        }
  ...
  const getLayout = (
    ...
    ) as React.ComponentType<{ data: any; forceTheme?: string }>;
