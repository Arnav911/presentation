    ...
    <>
      ...
      >
        ...
        <div className="flex h-full px-16 pb-16">
          ...
          <div className="flex-1 pr-16 flex items-center pt-8">
            <div className="w-full h-96 overflow-hidden">
              {slideData?.image ? (
                ...
                <div className="w-full h-full bg-gray-200 relative" style={{ backgroundColor: 'rgba(var(--theme-accent-rgb), 0.05)' }}>
                  {/* Building structure simulation */}
                  <div className="absolute inset-0 opacity-10" style={{ backgroundColor: 'var(--theme-text)' }}></div>
                  ...
                  <div className="absolute inset-0">
                    {[...Array(12)].map((_, i) => (
                      <div
                        key={i}
                        className="absolute w-full border-t border-gray-400 opacity-20"
                        style={{ top: `${(i + 1) * 8}%`, borderColor: 'var(--theme-text)' }}
                      ></div>
                    ))}
                  </div>
                  ...
                  <div className="absolute inset-0">
                    {[...Array(6)].map((_, i) => (
                      <div
                        key={i}
                        className="absolute h-full border-l border-gray-400 opacity-10"
                        style={{ left: `${(i + 1) * 16}%`, borderColor: 'var(--theme-text)' }}
                      ></div>
                    ))}
                  </div>
                  ...
                  <div className="absolute inset-0 grid grid-cols-4 gap-2 p-4">
                    {[...Array(32)].map((_, i) => (
                      <div
                        key={i}
                        className="opacity-40 rounded-sm border"
                        style={{ backgroundColor: 'rgba(var(--theme-accent-rgb), 0.2)', borderColor: 'rgba(var(--theme-accent-rgb), 0.3)' }}
                      ></div>
                    ))}
                  </div>

                  {/* Building edge highlight */}
                  <div className="absolute right-0 top-0 w-1 h-full opacity-30" style={{ backgroundColor: 'var(--theme-bg)' }}></div>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Right side - Content */}
          <div className="flex-1 pl-16 flex flex-col justify-center text-left">
            {slideData?.title && (
              <h2 className="text-6xl font-bold mb-12 leading-tight" style={{ color: 'var(--theme-accent)' }}>
                {slideData?.title}
              </h2>
            )}

            {slideData?.content && (
              <div className="text-lg leading-relaxed font-normal max-w-lg" style={{ color: 'var(--theme-text)' }}>
                {slideData?.content}
              </div>
            )}
          </div>
