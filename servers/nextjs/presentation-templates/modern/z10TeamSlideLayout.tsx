  ...
  return (
    <>
      ...
      >
        ...
        {/* Main Content */}
        <div className="relative z-10 flex flex-col items-start justify-center h-full px-16 pt-24 pb-10 text-left">
          {/* Title */}
          <h1
            className="text-7xl font-bold mb-4 leading-tight"
            style={{ letterSpacing: "-0.03em", color: 'var(--theme-accent)' }}
          >
            {slideData?.title}
          </h1>
          {/* Subtitle */}
          <p className="text-lg leading-relaxed font-normal mb-12 max-w-lg opacity-90">
            {slideData?.subtitle}
          </p>
          {/* Team Members Row */}
          <div className="flex flex-row w-full justify-between items-start gap-6 mt-2">
            {slideData?.teamMembers?.map((member, idx) => (
              <div
                key={idx}
                className="flex flex-col items-center rounded-lg shadow-sm px-6 pt-6 pb-4 w-1/4 min-w-[210px] max-w-[240px] mx-auto"
                style={{ minHeight: 340, backgroundColor: 'rgba(var(--theme-accent-rgb), 0.05)', border: '1px solid rgba(var(--theme-accent-rgb), 0.1)' }}
              >
                {/* Photo */}
                <div className="relative w-28 h-28 mb-4 rounded overflow-hidden flex items-center justify-center" style={{ backgroundColor: 'var(--theme-bg)', border: '2px solid rgba(var(--theme-accent-rgb), 0.2)' }}>
                ...
                {/* Name */}
                <div className="text-lg font-bold mb-1" style={{ color: 'var(--theme-accent)' }}>
                  {member.name}
                </div>
                {/* Position Badge */}
                <div className="text-xs font-semibold px-3 py-1 rounded-sm mb-2 uppercase tracking-wide" style={{ backgroundColor: 'var(--theme-accent)', color: 'var(--theme-bg)' }}>
                  {member.position}
                </div>
                {/* Description */}
                <div className="text-sm text-center mb-2 min-h-[48px] opacity-90" style={{ color: 'var(--theme-text)' }}>
                  {member.description}
                </div>
