  ...
  return (
    <>
      ...
      >
        ...
        {/* Content grid */}
        <div className="flex flex-col px-12 pb-16 h-full min-h-0 pt-6">
          <div className="grid grid-cols-[45%_55%] gap-12 items-start h-full">
            {/* Left heading and statement */}
            <div className="pt-8">
              <div className="text-[44px] leading-[1.1] font-semibold line-clamp-4" style={{ color: 'var(--text-heading-color, #111827)' }}>
                {slideData?.title}
              </div>

              <p className="mt-8 text-[15px] leading-[1.7] max-w-[500px] line-clamp-[8] overflow-hidden" style={{ color: 'var(--text-body-color, #6B7280)' }}>
                {slideData?.statement}
              </p>
            </div>

            {/* Right numbered points (up to 4) */}
            <div className="flex flex-col gap-6 pt-8 overflow-hidden h-full">
              {points.slice(0, 4).map((p, i) => (
                <div key={i} className="min-h-0 flex-1">
                  <div className="text-[22px] font-semibold line-clamp-2" style={{ color: 'var(--text-heading-color, #111827)' }}>{p.title}</div>
                  <p className="mt-2 text-[15px] leading-[1.6] line-clamp-4 overflow-hidden" style={{ color: 'var(--text-body-color, #6B7280)' }}>{p.body}</p>
                </div>
              ))}
            </div>
          </div>
