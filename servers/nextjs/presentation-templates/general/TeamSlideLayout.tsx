            ...
            >
                ...
                <div className="relative z-10 flex h-full px-8 sm:px-12 lg:px-20 pt-12 pb-8">
                    ...
                    <div className="flex-1 flex flex-col justify-center pr-8 space-y-4">
                        {/* Title */}
                        <h1 style={{ color: "var(--text-heading-color,#111827)" }} className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 leading-tight">
                            {slideData?.title || 'Our Team Members'}
                        </h1>

                        {/* Purple accent line */}
                        <div style={{ background: "var(--primary-accent-color,#9333ea)" }} className="w-16 h-1 bg-purple-600"></div>

                        {/* Company Description */}
                        <p style={{ color: "var(--text-body-color,#4b5563)" }} className="text-sm sm:text-base text-gray-700 leading-relaxed text-left">
                        ...
                        <div className={`grid ${getGridClasses(teamMembers.length)} w-full max-w-2xl`}>
                            {teamMembers.map((member, index) => (
                                <div key={index} className="text-center space-y-2">
                                    {/* Member Photo */}
                                    <div className="w-24 h-24 mx-auto rounded-lg overflow-hidden shadow-md" style={{ background: "var(--tertiary-accent-color,#e5e7eb)" }}>
                                        <img
                                    ...
                                    {/* Member Info */}
                                    <div>
                                        <h3 style={{ color: "var(--text-heading-color,#111827)" }} className="text-base font-semibold text-gray-900">
                                            {member.name}
                                        </h3>
                                        <p style={{ color: "var(--text-body-color,#4b5563)" }} className="text-sm font-medium text-gray-600 italic mb-2">
                                            {member.position}
                                        </p>
                                        <p style={{ color: "var(--text-body-color,#4b5563)" }} className="text-xs text-gray-600 leading-relaxed px-2 text-left">
                                            {member.description}
                                        </p>
                                    </div>
                                </div>
